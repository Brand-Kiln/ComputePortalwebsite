...
const MarketplaceFilters: React.FC<MarketplaceFiltersProps> = ({ onFilterChange }) => {
  ...
  const hardwareCategories = [
    { id: 'edge-smart-devices', label: 'Edge & Smart Devices', subcategories: ['IoT Gateways', 'Smart Sensors', 'Edge Computers'] },
    { id: 'accelerator-units', label: 'Accelerator Units', subcategories: ['GPU Cards', 'FPGA Boards', 'ASIC Miners'] },
    { id: 'storage-data-nodes', label: 'Storage & Data Nodes', subcategories: ['NAS Systems', 'SSD Arrays', 'Distributed Storage'] },
    { id: 'custom-systems', label: 'Custom Systems', subcategories: ['Custom Rigs', 'Modular PCs', 'Specialized Hardware'] }
  ];

  const softwareCategories = [
    { id: 'ai-models-apis', label: 'AI Models & APIs', subcategories: ['GPT Models', 'Image Recognition', 'NLP Services'] },
    { id: 'data-processing', label: 'Data Processing', subcategories: ['Big Data', 'Analytics', 'ETL Pipelines'] },
    { id: 'oracles-automation', label: 'Oracles & Automation', subcategories: ['Price Feeds', 'Smart Contracts', 'Triggers'] },
    { id: 'blockchain-services', label: 'Blockchain Services', subcategories: ['Node Hosting', 'dApp Backend', 'Deployment'] }
  ];
              ...
              <div className="ml-6 space-y-1">
                {category.subcategories.map((sub: string, idx: number) => (
                  <div key={idx} className="flex items-center space-x-2">
                    ...
                    <label htmlFor={`${category.id}-${idx}`} className="text-xs text-muted-foreground cursor-pointer">
                      {sub}
                    </label>
                  </div>
                ))}
